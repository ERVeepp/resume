...
- LangGraph 可选引擎（2026.08）：AgentEngine 统一接口，自研 loopEngine 与 graphEngine（@langchain/langgraph）按子 Agent 切换；SqliteSaver 持久化 checkpoint（跨进程可恢复）+ 节点级断点续跑（原生 interrupt()/resume，verify-resume 端到端验证：模拟重启后从断点续跑、不重跑已完成的节点）+ subgraph 跨子图 State 共享；收益评估用数据支撑选型（loop 0.2ms vs graph 16.6ms，短任务走 loop、长任务切 graph）
...
- AI Coding Pipeline（batch-generate，2026.08）：批量变体生成流水线——模板+参数 → 变体 → 语法/契约门禁 → 自动独立分支/PR，输出首次通过率；支持 --llm 真生成（OpenAI 兼容）、Spec 契约驱动生成（generate_user_skill 带 contract 外约门禁 + 落盘 golden）
- CI 变更门禁：batch-generate 变体门禁 + regression-all 黄金集回归（历史请求退化 exit 1），把 AI 生成代码纳入发布前自动校验
