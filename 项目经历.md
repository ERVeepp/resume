...
## 广告实时竞价实验室（ad-tech-labs）

**时间**：2026.07 - 至今 ｜ **角色**：独立开发

**技术栈**：Go（OpenRTB 2.5 Bidder）+ Kafka + Flink 1.20（SQL/PyFlink）+ ClickHouse + NestJS + Docker Compose

**项目描述**：从零搭建程序化广告「真实页面 → Header Bidding → Go Bidder → 实时指标」完整验证闭环。Go Bidder 实现 OpenRTB 2.5 标准竞价接口（POST /bid 解析 BidRequest → 素材匹配 → BidResponse），竞价日志异步写入 Kafka。Flink 1.20 集群消费 bid-logs，5 秒滚动窗口（service + imp_id 分组）聚合竞价指标——bid_requests / bid_wins / avg_price / win_rate，经 Kafka bid-metrics 回流 ClickHouse（Kafka Engine 表 + 物化视图）落地 ad_realtime.bid_metrics。核心是事件时间语义：声明 Watermark 允许 5 秒乱序，解决竞价日志乱序导致的窗口错配。NestJS 后台提供素材 CRUD、Win Notice 接收与 Dashboard。

**项目业绩**：
- 打通「Header Bidding → Go Bidder → Kafka → Flink → ClickHouse」端到端实时链路
- Flink 事件时间 + Watermark 处理竞价日志乱序，5s 窗口输出 RPS/胜率/eCPM 类实时指标
