...
2026.08 完成 RAG 知识增强改造：KnowledgeAgent 从飞书 API 关键词搜索升级为向量库语义检索——文档分块 → Embedding（Ollama）→ Qdrant 存储（993 chunks），检索链路向量召回 + 引用溯源（title/source 回传），多租户 productId payload 过滤预留。chat 与 embedding 模型分离支持多模型。配套数据准备脚本（扫描 docs → 分块 → 向量化入库）+ 黄金集检索评测。重排、混合检索、成本追踪列入迭代计划（bge-m3 换模型 / bge-reranker 精排）。
