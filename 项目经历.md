...
2026.08 完成 RAG 知识增强改造：KnowledgeAgent 从飞书 API 关键词搜索升级为向量库语义检索——文档分块（512 token + 10% 重叠）→ Embedding → Qdrant 存储，检索链路 top50 召回 + 重排 top5 + 引用溯源，多租户 productId payload 过滤实现文档级权限隔离。引入 LangChain 的文档加载/分块/Embedding/向量库组件（选择性用库，Agent 循环与路由保持自研），chat 与 embedding 模型分离支持多模型热切换。token 成本追踪记录每次调用的模型/用量/费用，量化模型分级省钱策略。
...
- 成本可观测：token 用量 + 费用实时追踪，量化模型分级省钱策略
- LangChain 选型实践：RAG 用框架库，Agent 循环/路由自研，各取所长
- skill 回归测试：设计契约 + 黄金集 + LLM-as-Judge 四层方案，AI 生成脚本改版可回归验证
